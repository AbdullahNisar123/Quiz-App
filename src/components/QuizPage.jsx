...
export default function QuizPage({ qustionNum, answer, options ,onAnswer}) {
      ...
      <div className="space-y-4">
        {
          options.map((option, index) => {
            ...
            return (
              <button key={index} className="w-full bg-purple-400/30 hover:bg-purple-400/50 py-3 px-4 rounded-xl transition" onClick={()=>onAnswer(index)}>
                {option}
              </button>
            )
          })
